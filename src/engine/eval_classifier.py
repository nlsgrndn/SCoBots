...
def eval_classifier(cfg):
    ...
    x, y = AtariDataCollector.collect_z_what_data_reshaped(cfg, dataset_mode, data_subset_mode, only_collect_first_image_of_consecutive_frames=True)
    ...
    df.to_csv(f"{cfg.resume_ckpt.rsplit('/', 1)[0]}/eval_classifier.csv", header=True, index=True)
