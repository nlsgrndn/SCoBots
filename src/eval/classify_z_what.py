...
def evaluate_z_what(arguments, z_what, labels, n, cfg, title="", method="pca"):
    ...
    # Filter the data to only include relevant labels
    relevant = torch.zeros(labels.shape, dtype=torch.bool)
    for rl in relevant_labels:
        relevant |= labels == rl # |= is the bitwise OR operator
    ...
    create_ridge_classifiers(cfg, relevant_labels, test_x, test_y, few_shot_accuracy, z_what_by_game, labels_by_game)

    y, results, centroids, centroid_label = nearest_neighbor_computation(z_what, labels, relevant_labels, test_x, test_y, train_x, few_shot_accuracy)
    ...
    if arguments['edgecolors']:
        edgecolor_visualization(n, relevant_labels, folder, pca_path, label_list, y, centroid_label, colors, sorted, z_what_emb, centroid_emb)
    else:
        non_edgecolor_visualization(cfg, relevant_labels, folder, pca_path, label_list, y, centroid_label, colors, sorted, z_what_emb, centroid_emb, dim_name)
    
    return results, pca_path, few_shot_accuracy

def non_edgecolor_visualization(cfg, relevant_labels, folder, pca_path, label_list, y, centroid_label, colors, sorted, z_what_emb, centroid_emb, dim_name):
...
def edgecolor_visualization(n, relevant_labels, folder, pca_path, label_list, y, centroid_label, colors, sorted, z_what_emb, centroid_emb):
...
def nearest_neighbor_computation(z_what, labels, relevant_labels, test_x, test_y, train_x, few_shot_accuracy):
    ...
    return y,results,centroids,centroid_label

def create_ridge_classifiers(cfg, relevant_labels, test_x, test_y, few_shot_accuracy, z_what_by_game, labels_by_game):
