...
label_list_pong = ["no_label"] + sorted(list(MAX_NB_OBJECTS_HUD_PONG.keys()))
...
label_list_boxing = ["no_label"] + sorted(list(MAX_NB_OBJECTS_HUD_BOXING.keys()))
...
def filter_relevant_boxes(game, boxes_batch, boxes_gt):
    if "MsPacman" in game:
        ...
        return [box_bat[box_bat[:, 1] > 21 / 128 & box_bat[:, 1] < 118 / 128] for box_bat in boxes_batch]
    elif "Boxing" in game:
