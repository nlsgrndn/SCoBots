...
def create_dataset_using_ocatari():
    ...
    nr_of_sequences_for_folder = {"train": 16, "test": 4, "validation": 4}
    max_sequences = nr_of_sequences_for_folder[args.folder]
    data_base_folder = "../consecutive_atari_data"
    mode_base_folder = "../consecutive_atari_data"
    ...
    pbar = tqdm(total=max_sequences)
    REQ_CONSECUTIVE_IMAGE = 20
    MAX_DESIRED_SEQUENCE_LENGTH = 128
    image_count = 0
    sequence_count = 0
    consecutive_images = []
    ...
    while True:
        ...
        consecutive_images.append(obs)
        consecutive_images_info.append(info)

        if done or truncated or len(consecutive_images) >= MAX_DESIRED_SEQUENCE_LENGTH:
            space_stack = []
            resize_stack = []
            for i, (frame, img_info) in enumerate(zip(consecutive_images, consecutive_images_info)):
                # save original RGB
                space_stack.append(frame)
                img = Image.fromarray(frame, 'RGB')
                img.save(f'{rgb_folder}/{sequence_count:05}_{i}.png')

                # save resized BGR(?)
                frame_space = Image.fromarray(frame[:, :, ::-1], 'RGB').resize((128, 128), Image.ANTIALIAS)
                resize_stack.append(np.array(frame_space))
                frame_space.save(f'{bgr_folder}/{sequence_count:05}_{i}.png')

                # save the bounding boxes
                bb.save(args, frame_space, img_info, f'{bb_folder}/{sequence_count:05}_{i}.csv',
                        visualizations_bb)
            ...
            # save the flow
            if args.flow:
                flow.save(space_stack, f'{flow_folder}/{sequence_count:05}_{{}}.pt', visualizations_flow)
            
            # save the mode
            mode.save(space_stack, f'{mode_folder}/{sequence_count:05}_{{}}.pt', visualizations_mode,
                          mode=root_mode, space_frame=resize_stack, n_last_frames=len(consecutive_images))
            pbar.update(1)
            image_count += len(consecutive_images)
            sequence_count += 1
            consecutive_images, consecutive_images_info = [], []

            obs, reward, done, truncated, info = some_steps(agent) # reset env and take some steps
