...
import numpy as np
import os
from scobots.scobi import Environment
...
def main():
    ...
    if rgb_exp:
        ...
        if not opts.use_checkpoint:
            ...
            # load checkpoint for every env in SubprocVecEnv
            #train_env = VecNormalize.load(Path(ckpt_path, last_vecnorm), train_env)


            #eval_env = VecNormalize.load(Path(ckpt_path, last_vecnorm), eval_env)

            #eval_env = Environment(env_str, 
            #                focus_file=pruned_ff_name, 
            #                hide_properties=hide_properties, 
            #                draw_features=True, # implement feature attribution
            #                reward=0) #env reward only for evaluation
            #_, _ = eval_env.reset()
            #dummy_vecenv = DummyVecEnv([lambda :  eval_env])
            ##print(obs)
            #
            #eval_env = VecNormalize.load(Path(ckpt_path, last_vecnorm), dummy_vecenv)
            #eval_env.training = False
            #eval_env.norm_reward = False
